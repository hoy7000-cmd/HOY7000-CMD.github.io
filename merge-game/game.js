...
(() => {
  ...
  class GameState {
    ...
    merge(fromRow, fromCol, toRow, toCol) {
      ...
      let rescuePair = null;
      if (this.isStuck()) {
        // Endless mode: when board is deadlocked, inject a guaranteed merge pair.
        rescuePair = this.injectRescuePair();
      }
      return {
        ok: true,
        mergedLevel,
    ...
    hasAnyMergePair() {
      ...
      for (const level of this.grid) {
        ...
        if (counts.get(level) >= 2) return true;
      }
      return false;
    }

    isStuck() {
      // In this game, no equal-level pair means no legal merge move at all,
      // even if empty cells still exist.
      return !this.hasAnyMergePair();
    }
    ...
    injectRescuePair() {
      ...
      // Replace two lowest tiles with level-1 pair so player can always continue.
      this.grid[first.idx] = 1;
      this.grid[second.idx] = 1;
      return [
        { row: Math.floor(first.idx / this.gridSize), col: first.idx % this.gridSize },
        { row: Math.floor(second.idx / this.gridSize), col: second.idx % this.gridSize }
      ];
    }
  }
  ...
  class Renderer {
    ...
    draw(now) {
      ...
      this.mergeEffects = this.mergeEffects.filter((e) => now - e.start <= CONFIG.mergeAnimDuration);
      ...
      this.floatingTexts = this.floatingTexts.filter((item) => now - item.start <= CONFIG.popTextDuration);
    }
  }
  ...
  class GameApp {
    ...
    tryDrop(x, y) {
      ...
      this.renderer.addMergeEffect(result.target.row, result.target.col);
      this.renderer.addFloatingScore(result.target.row, result.target.col, result.gained);
      this.audio.playMerge(result.mergedLevel);
      if (result.rescuePair) {
        for (const pair of result.rescuePair) {
          this.renderer.addMergeEffect(pair.row, pair.col);
        }
      }
      this.ui.refresh();
    }

    ensureEndlessPlayable() {
      if (!this.state.isStuck()) return;
      const rescuePair = this.state.injectRescuePair();
      if (!rescuePair) return;
      for (const pair of rescuePair) {
        this.renderer.addMergeEffect(pair.row, pair.col);
      }
      this.ui.refresh();
    }
